...
export default function AdminItem({_id,name,image,dob,pob,description,address,country,date,career,prize,evaluate,period,medal,position,honnors,url,imageVideo,likeVideo,dislikeVideo,like, dislike, star,data,setData,onEditItem}){
    ...
    let result = await fetch(`https://history-mindx1.herokuapp.com/api/history/${_id}`,{
      method:"put",
    ...
    return(
        ...
        <div>
            <List.Item
                    ...
                    {editing ? <input type="text" placeholder="pob" onChange={(event)=>setNewPob(event.target.value)} />:<div></div>}<br/><br/>
                    ...
                    {editing ? <input type="text" placeholder="date" onChange={(event)=>setNewDate(event.target.value)} />:<div></div>}<br/><br/>
